 ...
 */
public class CloudSimExampleMax {
    ...
    public static void main(String[] args) {
        ...
        Log.println("Starting CloudSimExampleMax...");
        ...
        try {
            ...
            // CPU time
            vmid++;
            Vm vm2 = new Vm(vmid, brokerId, mips * 4, pesNumber, ram, bw, size, vmm, new CloudletSchedulerTimeShared());
            ...
            // Distribuir los Cloudlets basado en el performance de las VMs
            distribuirCloudlets(broker, cloudletList, vmList);
            ...
            Log.println("CloudSimExampleMax finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void distribuirCloudlets(DatacenterBroker broker, List<Cloudlet> cloudletList, List<Vm> vmList) {
        // Crear un mapa donde guardar el performance de cada VM
        Map<Vm, Double> vmPerformance = new HashMap<>();

        // Calcular el performance de cada VM basado en los MIPS
        for (Vm vm : vmList) {
            vmPerformance.put(vm, (double) vm.getMips());
        }

        // Acomodar las VMs basdao en su respectivo performance
        List<Map.Entry<Vm, Double>> sortedVms = new ArrayList<>(vmPerformance.entrySet());
        sortedVms.sort((a, b) -> b.getValue().compareTo(a.getValue())); // Orden descendiente

        // Asignar los Cloudlets a cada VM basado en el performance de las mismas
        for (int i = 0; i < cloudletList.size(); i++) {
            Cloudlet cloudlet = cloudletList.get(i);
            Vm selectedVm = sortedVms.get(i % sortedVms.size()).getKey(); // Asignacion usando el algoritmo Round-Robin
